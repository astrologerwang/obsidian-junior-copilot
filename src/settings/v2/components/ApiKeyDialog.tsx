...
function ApiKeyModalContent({ onClose }: ApiKeyModalContentProps) {
    ...
    const [verifyingProviders, setVerifyingProviders] = useState<Set<SettingKeyProviders>>(
        new Set()
    );
    ...
    const verifyModel = async () => {
        ...
        try {
            ...
            const existingModel = settings.activeModels.find(
                (model) =>
                    model.name === selectedModel.name && model.provider === selectedModel.provider
            );
                ...
                <div className="tw-space-y-4">
                    {providers.map((item: ProviderKeyItem) => (
                        <React.Fragment key={item.provider}>
                                ...
                                <div className="tw-flex tw-flex-row tw-items-center tw-gap-2">
                                    ...
                                    <div className="tw-w-[72px]">
                                        {!item.isVerified ? (
                                            <Button
                                                onClick={() =>
                                                    verifyApiKey(item.provider, item.apiKey)
                                                }
                                                disabled={
                                                    !item.apiKey || verifyingProviders.size > 0
                                                }
                                                variant="secondary"
                                                size="sm"
                            ...
                            <Collapsible
                                open={expandedProvider === item.provider}
                                className="tw-mt-2"
                            >
                                        ...
                                        >
                                            <div>
                                                ...
                                                <div className="tw-mt-1 tw-text-xs">
                                                    ...
                                                    {modelsByProvider[item.provider] &&
                                                        modelsByProvider[item.provider]!.length ===
                                                            0 && (
                                                            <div className="tw-p-1 tw-text-muted">
                                                                No models available for this
                                                                provider.
                                                            </div>
                                                        )}
                                                    {modelsByProvider[item.provider] ===
                                                        ...
                                                        loadingProvider !== item.provider && (
                                                            <div className="tw-p-1 tw-text-muted">
                                                                Click to load models or expand to
                                                                try again if API key was changed.
                                                            </div>
                                                        )}
                                                </div>
                                            </div>
